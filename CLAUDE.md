...
Official Go client library for the proof.holdings API, providing typed struct returns, context.Context support, and generic HTTP helpers for all 25 resource groups.
...
### Client
- `client.go` - `Client` struct with 25 resource fields, `NewClient()` constructor, `ClientOption` functional options (`WithBaseURL`, `WithTimeout`, `WithMaxRetries`), `WaitOptions` struct
...
- `types.go` - 1,188 lines of Go structs generated from OpenAPI 3.1 schemas. Includes JSON struct tags, pointer types for optional fields, and type aliases for enums. Notable typed structs: `PaginationResponse` (with `HasMore`, `NextCursor`, `Total`), `ActionContext`, `WebhookPayload`. Enum constants include `ActionTypeConfirmation`
...
- `authorizations.go` - Authorizations (Create, Retrieve, List, Revoke via `delWithBodyAs`, Export)
- `hitl.go` - HITL configs (Create, Get, List, Update, Delete/Archive)
- `confirmations.go` - Confirmations (Create, Get, List)
...
- `resources_test.go` - Resource method tests covering all 25 resource types: request building, URL construction, query parameters, typed response unmarshalling
