...
func TestVerificationRequests_GetProofs(t *testing.T) {
	client, _ := resourceServer(t, "GET", "/api/v1/verification-requests/vr_123/proofs", map[string]any{
		"proofs": []any{
			map[string]any{"type": "phone", "identifier": "+37069199199", "channel": "whatsapp", "proof_token": "jwt_proof"},
		},
	})
	result, err := client.VerificationRequests.GetProofs(context.Background(), "vr_123")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	proofs, ok := result["proofs"].([]any)
	if !ok || len(proofs) != 1 {
		t.Errorf("expected 1 proof, got %v", result["proofs"])
	}
}
...
func TestAuthorizations_Create(t *testing.T) {
	client, cap := resourceServer(t, "POST", "/api/v1/authorizations", map[string]any{
		"id": "auth_123", "status": "pending", "phone": "+15551234567",
		"channel": "whatsapp", "scope": "confirmations", "business_name": "Acme Corp",
		"authorization_url": "https://verify.proof.holdings/auth/auth_123",
		"expires_at": "2025-04-01T00:00:00Z", "created_at": "2025-01-01T00:00:00Z",
	})
	result, err := client.Authorizations.Create(context.Background(), map[string]any{
		"phone": "+15551234567", "channel": "whatsapp", "business_name": "Acme Corp",
	})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	assertBodyField(t, cap, "phone", "+15551234567")
	assertBodyField(t, cap, "channel", "whatsapp")
	assertBodyField(t, cap, "business_name", "Acme Corp")
	if result.ID != "auth_123" {
		t.Errorf("want ID auth_123, got %s", result.ID)
	}
}

func TestAuthorizations_Retrieve(t *testing.T) {
	client, _ := resourceServer(t, "GET", "/api/v1/authorizations/auth_123", map[string]any{
		"id": "auth_123", "status": "active", "phone": "+15551234567",
		"channel": "whatsapp", "scope": "confirmations", "business_name": "Acme Corp",
		"confirmations_sent": float64(5), "expires_at": "2025-04-01T00:00:00Z",
		"created_at": "2025-01-01T00:00:00Z", "updated_at": "2025-01-01T00:00:00Z",
	})
	result, err := client.Authorizations.Retrieve(context.Background(), "auth_123")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if result.ID != "auth_123" {
	...
	if result.Status != "active" {
		t.Errorf("want status active, got %s", result.Status)
	}
}

func TestAuthorizations_Retrieve_EncodesSpecialChars(t *testing.T) {
	var rawPath string
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		rawPath = r.URL.EscapedPath()
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(map[string]any{
			"id": "auth/special", "status": "active", "phone": "+1",
			"channel": "sms", "scope": "confirmations", "business_name": "X",
			"confirmations_sent": float64(0), "expires_at": "2025-04-01T00:00:00Z",
			"created_at": "2025-01-01T00:00:00Z", "updated_at": "2025-01-01T00:00:00Z",
		})
	}))
	defer srv.Close()
	client, _ := NewClient("pk_test_123", WithBaseURL(srv.URL), WithMaxRetries(0))
	_, _ = client.Authorizations.Retrieve(context.Background(), "auth/special&id")
	if !strings.Contains(rawPath, "auth%2Fspecial") {
		t.Errorf("path slash not encoded: %s", rawPath)
	}
}

func TestAuthorizations_List(t *testing.T) {
	client, cap := resourceServer(t, "GET", "/api/v1/authorizations", map[string]any{
		"data": []any{}, "pagination": map[string]any{"page": float64(1), "limit": float64(20), "total": float64(0), "total_pages": float64(0)},
	})
	_, err := client.Authorizations.List(context.Background(), map[string]string{"status": "active", "channel": "whatsapp"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	assertQuery(t, cap, "status", "active")
	assertQuery(t, cap, "channel", "whatsapp")
}

func TestAuthorizations_Revoke(t *testing.T) {
	client, cap := resourceServer(t, "DELETE", "/api/v1/authorizations/auth_123", map[string]any{
		"id": "auth_123", "status": "revoked", "phone": "+15551234567",
		"channel": "whatsapp", "scope": "confirmations", "business_name": "Acme Corp",
		"confirmations_sent": float64(0), "expires_at": "2025-04-01T00:00:00Z",
		"created_at": "2025-01-01T00:00:00Z", "updated_at": "2025-01-01T00:00:00Z",
	})
	result, err := client.Authorizations.Revoke(context.Background(), "auth_123", map[string]any{
		"reason": "No longer needed",
	})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	assertBodyField(t, cap, "reason", "No longer needed")
	if result.Status != "revoked" {
		t.Errorf("want status revoked, got %s", result.Status)
	}
}

func TestAuthorizations_Export(t *testing.T) {
	client, cap := resourceServer(t, "GET", "/api/v1/authorizations/export", map[string]any{
		"data": []any{}, "total": float64(0),
	})
	_, err := client.Authorizations.Export(context.Background(), map[string]string{"format": "json", "status": "active"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	assertQuery(t, cap, "format", "json")
	assertQuery(t, cap, "status", "active")
}
...
func TestConfirmations_Create(t *testing.T) {
	client, cap := resourceServer(t, "POST", "/api/v1/confirmations", map[string]any{
		"id": "conf_123", "status": "pending", "hitl_id": "hitl_123",
		"message": "Deploy v2.3?", "timeout_seconds": float64(1800),
		"timeout_at": "2026-03-10T00:00:00Z", "environment": "test",
		"created_at": "2026-03-08T00:00:00Z", "updated_at": "2026-03-08T00:00:00Z",
	})
	result, err := client.Confirmations.Create(context.Background(), map[string]any{
		"hitl_id": "hitl_123", "message": "Deploy v2.3?",
	})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	assertBodyField(t, cap, "hitl_id", "hitl_123")
	assertBodyField(t, cap, "message", "Deploy v2.3?")
	if result.ID != "conf_123" {
		t.Errorf("want id conf_123, got %s", result.ID)
	}
}

func TestConfirmations_Retrieve(t *testing.T) {
	client, _ := resourceServer(t, "GET", "/api/v1/confirmations/conf_123", map[string]any{
		"id": "conf_123", "status": "pending", "hitl_id": "hitl_123",
		"message": "Deploy?", "timeout_seconds": float64(1800),
		"timeout_at": "2026-03-10T00:00:00Z", "environment": "test",
		"created_at": "2026-03-08T00:00:00Z", "updated_at": "2026-03-08T00:00:00Z",
	})
	result, err := client.Confirmations.Retrieve(context.Background(), "conf_123")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if result.Status != "pending" {
		t.Errorf("want status pending, got %s", result.Status)
	}
}

func TestConfirmations_List(t *testing.T) {
	client, cap := resourceServer(t, "GET", "/api/v1/confirmations", map[string]any{
		"data": []any{}, "pagination": map[string]any{"page": float64(1), "limit": float64(20), "total": float64(0), "total_pages": float64(0)},
	})
	_, err := client.Confirmations.List(context.Background(), map[string]string{
		"status": "pending", "hitl_id": "hitl_123",
	})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	assertQuery(t, cap, "status", "pending")
	assertQuery(t, cap, "hitl_id", "hitl_123")
}
...
func TestHitl_Create(t *testing.T) {
	client, cap := resourceServer(t, "POST", "/api/v1/hitl", map[string]any{
		"id": "hitl_123", "name": "Ops Approvals", "status": "active",
		"channels": []any{}, "timeout_seconds": float64(1800),
		"created_at": "2026-03-08T00:00:00Z", "updated_at": "2026-03-08T00:00:00Z",
	})
	result, err := client.Hitl.Create(context.Background(), map[string]any{
		"name": "Ops Approvals", "timeout_seconds": float64(1800),
	})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	assertBodyField(t, cap, "name", "Ops Approvals")
	if result.ID != "hitl_123" {
		t.Errorf("want ID hitl_123, got %s", result.ID)
	}
}

func TestHitl_Retrieve(t *testing.T) {
	client, _ := resourceServer(t, "GET", "/api/v1/hitl/hitl_123", map[string]any{
		"id": "hitl_123", "name": "Ops Approvals", "status": "active",
		"channels": []any{}, "timeout_seconds": float64(1800),
		"created_at": "2026-03-08T00:00:00Z", "updated_at": "2026-03-08T00:00:00Z",
	})
	result, err := client.Hitl.Retrieve(context.Background(), "hitl_123")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if result.ID != "hitl_123" {
...
func TestHitl_List(t *testing.T) {
	client, cap := resourceServer(t, "GET", "/api/v1/hitl", map[string]any{
		"data": []any{}, "pagination": map[string]any{"page": float64(1), "limit": float64(20), "total": float64(0), "total_pages": float64(0)},
	})
	_, err := client.Hitl.List(context.Background(), map[string]string{"status": "active"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	assertQuery(t, cap, "status", "active")
...
func TestHitl_Update(t *testing.T) {
	client, cap := resourceServer(t, "PATCH", "/api/v1/hitl/hitl_123", map[string]any{
		"id": "hitl_123", "name": "Updated Approvals", "status": "active",
		"channels": []any{}, "timeout_seconds": float64(3600),
		"created_at": "2026-03-08T00:00:00Z", "updated_at": "2026-03-08T00:00:00Z",
	})
	result, err := client.Hitl.Update(context.Background(), "hitl_123", map[string]any{
		"name": "Updated Approvals", "timeout_seconds": float64(3600),
	})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	assertBodyField(t, cap, "name", "Updated Approvals")
	if result.ID != "hitl_123" {
		t.Errorf("want ID hitl_123, got %s", result.ID)
	}
}

func TestHitl_Delete(t *testing.T) {
	client, _ := resourceServer(t, "DELETE", "/api/v1/hitl/hitl_123", map[string]any{
		"id": "hitl_123", "name": "Ops Approvals", "status": "archived",
		"channels": []any{}, "timeout_seconds": float64(1800),
		"created_at": "2026-03-08T00:00:00Z", "updated_at": "2026-03-08T00:00:00Z",
	})
	result, err := client.Hitl.Delete(context.Background(), "hitl_123")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if result.Status != "archived" {
		t.Errorf("want status archived, got %s", result.Status)
	}
}

func TestHitl_RequestAuthorization(t *testing.T) {
	client, _ := resourceServer(t, "POST", "/api/v1/hitl/hitl_123/authorize", map[string]any{
		"hitl_id": "hitl_123", "authorizations": []any{}, "message": "Sent",
	})
	result, err := client.Hitl.RequestAuthorization(context.Background(), "hitl_123")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if result.HitlID != "hitl_123" {
		t.Errorf("want hitl_id hitl_123, got %s", result.HitlID)
	}
}

func TestHitl_CreateChatIdDiscovery(t *testing.T) {
	client, _ := resourceServer(t, "POST", "/api/v1/hitl/chat-id-discovery", map[string]any{
		"token": "abc123", "deep_link": "https://t.me/bot?start=chatid_abc123",
		"qr_code": "data:image/png;base64,...", "expires_in": float64(600),
	})
	result, err := client.Hitl.CreateChatIdDiscovery(context.Background())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if result.Token != "abc123" {
		t.Errorf("want token abc123, got %s", result.Token)
	}
	if result.ExpiresIn != 600 {
		t.Errorf("want expires_in 600, got %d", result.ExpiresIn)
	}
}

func TestHitl_PollChatIdDiscovery(t *testing.T) {
	client, _ := resourceServer(t, "GET", "/api/v1/hitl/chat-id-discovery/abc123", map[string]any{
		"status": "completed", "chat_id": "999888777",
	})
	result, err := client.Hitl.PollChatIdDiscovery(context.Background(), "abc123")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	if result.ChatID == nil || *result.ChatID != "999888777" {
		t.Errorf("want chat_id 999888777, got %v", result.ChatID)
	}
}
