...
// GetProofs returns proof tokens for verified assets in a verification request.
func (vr *VerificationRequests) GetProofs(ctx context.Context, id string) (map[string]any, error) {
	return getAs[map[string]any](vr.http, ctx, "/api/v1/verification-requests/"+url.PathEscape(id)+"/proofs", nil)
}
